...
function Wordmark({ size = 18, color }) {
  return (
    <p style={{ fontFamily: FONT, fontWeight: 700, fontSize: size, letterSpacing: "-0.03em", color: color || C.text }}>
...
function MapPicker({ initialLat, initialLng, onConfirm, onClose }) {
  ...
  const [areaName, setAreaName] = useState("Tap on the map to drop pin")
  ...
  function useGPS() {
    navigator.geolocation.getCurrentPosition(
      ...
      () => alert("Location denied. Tap on the map instead.")
    )
  }
    ...
    <div className="fixed inset-0 z-[60] flex flex-col" style={{ background: C.bg, fontFamily: FONT }}>
      <div className="flex items-center gap-3 px-5 py-4 border-b" style={{ borderColor: C.border, background: C.surface }}>
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-medium" style={{ color: C.textMuted }}>Pin location</p>
          <p className="font-semibold truncate" style={{ color: C.text, fontSize: 15 }}>{areaName}</p>
        </div>
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute bottom-24 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full text-xs font-medium shadow-2xl" style={{ background: C.surface, color: C.textMuted, border: `1px solid ${C.border}` }}>
          Tap the map or drag the pin
        </div>
      </div>
      <div className="px-5 py-4 border-t" style={{ borderColor: C.border, background: C.surface }}>
        <button onClick={confirm} disabled={confirming} className="w-full py-4 rounded-xl text-base font-semibold active:scale-[0.98] transition-all" style={{ background: C.text, color: C.bg }}>
...
function Onboarding({ onDone }) {
  ...
  function getLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setError("Location denied. Pick on map below."); setLocating(false) }
    )
  }
    ...
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: C.bg, fontFamily: FONT }}>
      {step === 1 && (
        <div className="flex-1 flex flex-col px-7 pt-16 pb-10">
          <div className="mb-2"><Wordmark size={22} /></div>
          <div className="flex-1 flex flex-col justify-center">
            <h1 style={{ color: C.text, fontSize: 56, fontWeight: 700, letterSpacing: "-0.04em", lineHeight: 1.02 }}>
              Rentals,<br />reimagined.
            </h1>
            <p style={{ color: C.textMuted, fontSize: 17, lineHeight: 1.5, marginTop: 16, maxWidth: 320, fontWeight: 400 }}>
              Direct from owners. No brokers. No commission. The honest rental platform in Lucknow.
            </p>
          </div>
          <div>
            <p style={{ color: C.textMuted, fontSize: 13, fontWeight: 500, marginBottom: 14 }}>What are you looking for?</p>
            <div className="flex flex-col gap-2.5">
              <button onClick={() => pickCategory("residential")} className="text-left rounded-2xl px-5 py-4 active:scale-[0.99] transition-all" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
              ...
              <button onClick={() => pickCategory("commercial")} className="text-left rounded-2xl px-5 py-4 active:scale-[0.99] transition-all" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
      ...
      {step === 2 && (
        <div className="flex-1 flex flex-col px-7 pt-16 pb-10">
          <button onClick={() => setStep(1)} className="text-2xl self-start mb-8" style={{ color: C.text }}>←</button>
          <div className="flex-1 flex flex-col justify-center">
            <h1 style={{ color: C.text, fontSize: 48, fontWeight: 700, letterSpacing: "-0.04em", lineHeight: 1.05 }}>
              Where are you<br />looking?
            </h1>
            <p style={{ color: C.textMuted, fontSize: 17, lineHeight: 1.5, marginTop: 16, maxWidth: 320 }}>
              We'll surface {category} properties closest to you first.
            </p>
            {error && <p style={{ color: "#FF6B6B", fontSize: 14, marginTop: 16 }}>{error}</p>}
          </div>
          <div className="flex flex-col gap-2.5">
            <button onClick={getLocation} disabled={locating} className="w-full py-4 rounded-xl text-base font-semibold active:scale-[0.98] transition-all" style={{ background: C.text, color: C.bg }}>
              {locating ? "Locating..." : "Use my location"}
            </button>
            <button onClick={() => setShowMapPicker(true)} className="w-full py-4 rounded-xl text-base font-semibold active:scale-[0.98] transition-all" style={{ background: C.surface, color: C.text, border: `1px solid ${C.border}` }}>
              Pick on map
            </button>
          </div>
        </div>
        ...
        {search.length === 0 && (
          <>
            ...
            <div className="px-5 pb-5 flex flex-wrap gap-2">
              {POPULAR.map(p => (
                <button key={p} onClick={() => pickArea(p, null, null)} className="px-4 py-2 rounded-full text-sm" style={{ background: C.surface, color: C.text, border: `1px solid ${C.border}` }}>{p}</button>
              ))}
            </div>
          </>
        )}
...
function ListingDetail({ listing, onClose, onSave, isSaved }) {
  ...
  const shareText = `Check out this place on Homie: ${listing.title} in ${listing.location} — ₹${Number(listing.price).toLocaleString()}/mo`

  async function handleShare() {
    if (navigator.share) {
      try { await navigator.share({ title: listing.title, text: shareText, url: shareUrl }); return } catch {}
    }
    setShowShare(true)
  }
  function copyLink() {
  ...
  const tabs = listing.category === "commercial" ? ["overview", "amenities", "location"] : ["overview", "amenities", "neighborhood", "location"]
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: C.bg, fontFamily: FONT }}>
      ...
      <button onClick={() => onSave(listing)} className="absolute top-4 right-4 z-30 w-10 h-10 rounded-full flex items-center justify-center" style={{ background: isSaved ? C.accent : "rgba(28,28,30,0.85)", backdropFilter: "blur(20px)", color: isSaved ? "#000" : C.text }}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-6 pt-8 pb-6" style={{ background: C.bg }}>
            ...
            listing.amenities && listing.amenities.length > 0 ? (
              <div className="grid grid-cols-2 gap-2">
                {listing.amenities.map(a => (
                  <div key={a} className="px-4 py-3 rounded-xl text-sm font-medium" style={{ background: C.surface, color: C.text }}>{a}</div>
                ))}
              </div>
      ...
      <div className="px-5 py-4 border-t flex gap-3" style={{ borderColor: C.border, background: C.bg }}>
        <button
          onClick={handleShare}
        ...
        <button
          ...
          className="flex-1 py-4 rounded-xl text-base font-semibold active:scale-[0.98] transition-all"
      ...
      {showShare && (
        <div className="fixed inset-0 z-[70] flex items-end" style={{ fontFamily: FONT }} onClick={() => setShowShare(false)}>
          <div className="w-full rounded-t-3xl p-6 pb-10 flex flex-col gap-3" style={{ background: C.surface, border: `1px solid ${C.border}` }} onClick={e => e.stopPropagation()}>
            <div className="w-10 h-1 rounded-full mx-auto mb-2" style={{ background: C.border }} />
            <p style={{ color: C.text, fontSize: 17, fontWeight: 700, letterSpacing: "-0.02em" }}>Share listing</p>
            <button
              onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(shareText + "\n" + shareUrl)}`)}
              className="flex items-center gap-4 px-5 py-4 rounded-2xl active:scale-[0.99] transition-all"
              style={{ background: C.surfaceLift }}
            >
              <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "#25D366" }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="#fff"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.121.554 4.109 1.523 5.837L.057 23.428a.5.5 0 0 0 .515.572l5.761-1.513A11.943 11.943 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.802 9.802 0 0 1-4.98-1.359l-.357-.212-3.699.97.988-3.606-.233-.371A9.818 9.818 0 0 1 2.182 12C2.182 6.57 6.57 2.182 12 2.182S21.818 6.57 21.818 12 17.43 21.818 12 21.818z"/></svg>
              </div>
              <div className="text-left">
                <p style={{ color: C.text, fontSize: 15, fontWeight: 600 }}>Share on WhatsApp</p>
                <p style={{ color: C.textMuted, fontSize: 12 }}>Send to a friend</p>
              </div>
            </button>
            <button
              onClick={copyLink}
              ...
              style={{ background: C.surfaceLift }}
            >
              <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
                {copied
                  ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={C.accent} strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                  : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={C.text} strokeWidth="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
                }
              </div>
              <div className="text-left">
                <p style={{ color: copied ? C.accent : C.text, fontSize: 15, fontWeight: 600 }}>{copied ? "Copied!" : "Copy link"}</p>
                <p style={{ color: C.textMuted, fontSize: 12 }}>Paste anywhere</p>
              </div>
            </button>
          </div>
        </div>
...
function SwipeStack({ listings, onSwipe, onOpen }) {
  ...
  function handleStart(clientX) { if (dragState.animating) return; startX.current = clientX; offsetX.current = 0; isDragging.current = true; didDrag.current = false }
        ...
        >
          ...
          <div className="p-5">
            ...
            <div className="flex items-center justify-between mb-4 pt-3 border-t" style={{ borderColor: C.border }}>
              ...
              <button onClick={() => !didDrag.current && onOpen(listing)} onMouseDown={e => e.stopPropagation()} style={{ color: C.accent, fontSize: 12, fontWeight: 600 }}>Details →</button>
            </div>

            <div className="flex gap-2">
              <button onClick={() => buttonSwipe("left")} onMouseDown={e => e.stopPropagation()} className="flex-1 py-3.5 rounded-xl text-sm font-semibold active:scale-95 transition-all" style={{ background: C.bg, color: C.textMuted, border: `1px solid ${C.border}` }}>Skip</button>
              <button onClick={() => window.open("https://wa.me/91" + listing.phone)} onMouseDown={e => e.stopPropagation()} className="flex-1 py-3.5 rounded-xl text-sm font-semibold active:scale-95 transition-all" style={{ background: C.text, color: C.bg }}>Message</button>
              <button onClick={() => buttonSwipe("right")} onMouseDown={e => e.stopPropagation()} className="flex-1 py-3.5 rounded-xl text-sm font-semibold active:scale-95 transition-all" style={{ background: C.accent, color: "#000" }}>Save</button>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="relative flex-1 overflow-hidden" style={{ fontFamily: FONT }}>
      ...
      <div className="absolute top-4 left-4 right-4 px-4 py-3 rounded-xl z-10 flex items-center justify-between pointer-events-none" style={{ background: "rgba(28,28,30,0.85)", backdropFilter: "blur(20px)", border: `1px solid ${C.border}` }}>
        <p style={{ color: C.text, fontSize: 13, fontWeight: 600 }}>Map view</p>
        <p style={{ color: C.textMuted, fontSize: 12 }}>{pinned.length} pinned · {listings.length} total</p>
      </div>
    </div>
  )
}
...
function UploadForm({ onClose, onSuccess, defaultCategory }) {
  ...
  function getLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setError("Location denied. Pick on map manually."); setLocating(false) }
    )
  }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" style={{ background: "rgba(0,0,0,0.7)", backdropFilter: "blur(10px)", fontFamily: FONT }}>
      <div className="rounded-t-3xl sm:rounded-2xl w-full sm:max-w-lg max-h-[95vh] overflow-y-auto" style={{ background: C.bg }}>
        ...
        <div className="px-6 pb-8 pt-5 flex flex-col gap-5">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={() => setForm({ ...form, category: "residential" })} className="flex-1 py-3 rounded-xl text-sm font-semibold transition-all" style={{ background: form.category === "residential" ? C.text : C.surface, color: form.category === "residential" ? C.bg : C.textMuted, border: `1px solid ${C.border}` }}>Residential</button>
            <button type="button" onClick={() => setForm({ ...form, category: "commercial" })} className="flex-1 py-3 rounded-xl text-sm font-semibold transition-all" style={{ background: form.category === "commercial" ? C.text : C.surface, color: form.category === "commercial" ? C.bg : C.textMuted, border: `1px solid ${C.border}` }}>Commercial</button>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {AMENITIES.map(a => (
                <button key={a} type="button" onClick={() => toggleAmenity(a)} className="px-3 py-1.5 rounded-full text-xs font-medium transition-all" style={{
          ...
          <button onClick={submit} disabled={loading} className="w-full py-4 rounded-xl text-base font-semibold active:scale-[0.98] transition-all disabled:opacity-40" style={{ background: C.text, color: C.bg }}>
          ...
          {saved.length === 0 ? (
            ...
            <div className="px-5 pb-6 flex flex-col gap-2">
              {saved.map(l => {
                ...
                return (
                  <button key={l.id} onClick={() => setOpenListing(l)} className="rounded-2xl overflow-hidden flex text-left w-full active:scale-[0.99] transition-all" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
